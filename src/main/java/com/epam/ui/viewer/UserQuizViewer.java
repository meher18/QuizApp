...
public class UserQuizViewer {
    ...
    @Autowired
    public UserQuizService userQuizService;
    
    @Autowired
    public QuizService quizService;
    
    // to view all the quizzes participated by the user
...
//        UserQuizService userQuizService = AppContext.getApplicationContext().getBean(UserQuizService.class);
        ...
        Map<Integer, UserQuiz> quizzes = userQuizService.getAllUserQuizzes().entrySet()
                .stream()
                .filter(userUserQuizEntry -> 
                	userUserQuizEntry.getValue().getUser().getId() == UserSession.getLoggedUser().getId()
                )
...
//            LOGGER.debug(UserSession.getLoggedUser().getId());
    ...
    // to view all hosted quizzes by admin
