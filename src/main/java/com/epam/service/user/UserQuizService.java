...
public class UserQuizService {
	...
	public boolean checkIfQuizTaken(User loggedUser) {
		...
		int count = (int) userQuizLibrary.getUserQuizzesWithId().values().stream()
				// removed && UserSession.getLoggedUser() != null
				.filter(uQuiz -> uQuiz.getUser().getId() == loggedUser.getId()).count();
		if (count > 0) {
